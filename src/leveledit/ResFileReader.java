...
/**
 * Reads text files with different posts. Used for inifile & RILedit internal
 * file format.
 * <p>
 * The file format:
 * 
 * <br>
 * Posts are marked with [name] <br>
 * comments with # <br>
 * you can jump to post x, then parse word 1,2... <br>
 * or parse line 1,2...
 * 
 * <p>
 * TODO: Ok, it's very quick and dirty but it works...
 * 
 */
public class ResFileReader {
	...
	public String getNextWord() throws Exception {
		if (!nextWord())
			throw new Exception("getNextWord: end of line");
		return getWord();
	}
	...
	public int getNextWordAsInt() throws Exception {
		if (!nextWord())
			throw new Exception("getNextWordAsInt: end of line");
		return getWordAsInt();
	}
	...
	public boolean getNextWordAsBool() throws Exception {
		if (!nextWord())
			throw new Exception("getNextWordAsBool: end of line");
	...
	public int getWordAsInt() throws Exception {
		try {
			...
			throw new Exception("getWordAsInt: Word wasn't an int, found: "
					+ getWord());
		}
	}
	...
	public boolean getWordAsBool() throws Exception {
		if (getWord().compareTo("YES") == 0)
			return true;
		if (getWord().compareTo("NO") == 0)
			return false;
		throw new Exception("getWordAsBool: Wasn't YES or NO; Found: "
				+ getWord());
	}

	/**
	 * Get everything until line break, ignore separators.
	 * @return
	 */
	...
	public void gotoPost(String name, boolean readFirstLine) throws Exception {
		...
		while (true) {
			...
			if (nextChar() == false)
				throw new Exception("Couldn't find post " + name);
		}
	}
	...
	/**
	 * Read a map array in current post. Format is asciicoded integers separated
	 * by whitespace.
	 */
		...
		try {
			...
			for (int i = 0; i < y; i++) {
				for (int j = 0; j < x; j++) {
					...
					if (j == 0 && i == 0)
						map[i][j] = getWordAsInt();
					else
						map[i][j] = getNextWordAsInt();
